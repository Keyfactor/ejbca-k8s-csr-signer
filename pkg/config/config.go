...
type ServerConfig struct {
    ...
    ChainDepth                      int    `yaml:"chainDepth"`
}
...
func LoadConfig() (*ServerConfig, error) {
    ...
    if err != nil {
        configLog.Errorln("Ensure that a configmap was created containing config.yaml")
        return nil, err
    }
    ...
    configLog.Tracef("%s exists and contains %d bytes:\n %s", file, len(buf), buf)
